...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # raise NotImplementedError()
        positive_dx = dout.clone().detach()
        alpha_dout = self.alpha * dout
        negative_dx = alpha_dout.clone().detach()
        dx = torch.where(x > 0, positive_dx, negative_dx)
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        x = self.grad_cache["x"]
        e_x = torch.exp(-x)
        sigmoid_acti_func = 1 / (1 + e_x)
        dx = sigmoid_acti_func * (1 - sigmoid_acti_func) * dout
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        # raise NotImplementedError()
        _x_y = -x.gather(1, y.view(-1, 1))
        log_xk = torch.log(torch.exp(x).sum(dim=1))
        loss = _x_y + log_xk
        loss = loss.mean()
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        x_zeros.index_put_((torch.tensor(range(x.shape[0])), y), torch.tensor([1.]))
        # print("x_zeros", x_zeros)
        dx = ((torch.div(ex.T, ex_sum).T - x_zeros) * dout) / N
        # ========================
    ...
    def __init__(
        ...
        **kw,
        ...
        if activation == 'sigmoid':
            ...
            raise Exception('The activation function should be relu or sigmoid')
        layers.append(Linear(in_features, hidden_features[0]))
        layers.append(activation_func())
        if dropout > 0:
            layers.append(Dropout(dropout))
        len_hidd = len(hidden_features)
        for i in range(len_hidd - 1):
