...
class Trainer(abc.ABC):
    ...
    def fit(
        ...
        for epoch in range(num_epochs):
            ...
            # print("train_losses ", type(train_losses[0]))
            ...
            if checkpoints:
                ...
                # self.save_checkpoint("checkpoints/best_acc.pt")
...
class ClassifierTrainer(Trainer):
    ...
    def __init__(
        ...
        :param device: torch.device to run training on (CPU or GPU).
        """
        super().__init__(model, device, optimizer)
        self.optimizer = optimizer
        self.loss_fn = loss_fn
        self.device = device
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        # raise NotImplementedError()
        out = torch.log(self.model.predict_proba(X))
...
class LayerTrainer(Trainer):
    def __init__(self, model, loss_fn, optimizer):
        # ====== YOUR CODE: ======
        # raise NotImplementedError()
        super(LayerTrainer, self).__init__(model, loss_fn, optimizer)
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        _, out_classes = torch.max(out, dim=1)
        num_correct = torch.sum(out_classes == y)
        loss = loss.item()
