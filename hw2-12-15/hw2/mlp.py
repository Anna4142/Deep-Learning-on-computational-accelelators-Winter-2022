...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        for in_dim, out_dim in zip(all_dims[:-1], all_dims[1:]):
            if isinstance(nonlins[index], str):
                layer = ACTIVATIONS[nonlins[index]](**ACTIVATION_DEFAULT_KWARGS[nonlins[index]])
                # print(ACTIVATION_DEFAULT_KWARGS[nonlins[index]])
                layers += [nn.Linear(in_dim, out_dim, bias=True), layer]
            else:
                layer = nonlins[index]
                layers += [nn.Linear(in_dim, out_dim, bias=True), layer]
