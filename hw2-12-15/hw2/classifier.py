...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        # raise NotImplementedError()
        out = y_proba[:, self.positive_class] >= self.threshold
        return out.type(torch.int)
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # ====== YOUR CODE: ======
    # raise NotImplementedError()
    grid_range = torch.arange(-2, 3, dx)
    x1_grid, x2_grid = torch.meshgrid(grid_range, grid_range)
    x1 = torch.reshape(x1_grid, (-1, 1))
    x2 = torch.reshape(x2_grid, (-1, 1))
    x_cat = torch.cat((x1, x2), dim=1)
    y_hat = classifier.classify(x_cat)
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    # raise NotImplementedError()
    out = classifier.predict_proba(x)[:, 1].detach().numpy()
    gt = y.detach().numpy()
    fpr, tpr, thresh = roc_curve(gt, out)
    dis = fpr ** 2 + (tpr - 1) ** 2
    optimal_thresh_idx = torch.argmin(torch.tensor(dis)).item()
