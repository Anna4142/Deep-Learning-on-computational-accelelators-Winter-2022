...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        torch_zero = [torch.zeros(size=(1,))]
        self.momt = torch_zero * params_len
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # raise NotImplementedError()
            dp += self.reg * p
            momt1 = self.momentum * self.momt[i] - self.learn_rate * dp
            self.momt[i] = momt1
            p += momt1
            i += 1
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        params_len = len(self.params)
        torch_zero = [torch.zeros(size=(1,))]
        self.rpt = torch_zero * params_len
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # raise NotImplementedError()
            dp += self.reg * p
            rpt1 = self.decay * self.rpt[i] + (1 - self.decay) * torch.pow(dp, 2)
            self.rpt[i] = rpt1
            p -= (self.learn_rate / torch.sqrt((rpt1 + self.eps))) * dp
            i += 1
            # ========================
