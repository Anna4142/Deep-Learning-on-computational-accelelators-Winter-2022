...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # Forward pass
        y_pred, hidden_state = self.model(x, self.hidden_state)  # (B,S,V), (1,B,H)
        ...
        num_correct = (y_pred == y).sum().float()
...
class VAETrainer(Trainer):
    def train_batch(self, batch) -> BatchResult:
        ...
        loss, data_loss, kldiv_loss = self.loss_fn(x_recon, x, mu, logvar)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        # ========================
