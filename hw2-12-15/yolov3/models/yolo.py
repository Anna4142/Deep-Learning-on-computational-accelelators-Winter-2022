...
if __name__ == '__main__':
    ...
    x = torch.rand(1, 3, 640, 640, requires_grad=True).cuda()
    y = model(x)
    print(y[0].grad_fn)
    # tuple_res = (y[0], y[1][0].clone().detach().requires_grad_(True), y[1][1].clone().detach().requires_grad_(True),
    #              y[1][2].clone().detach().requires_grad_(True))
    g = make_dot(y[0], params=dict(list(model.named_parameters()) + [('x', x)]))
